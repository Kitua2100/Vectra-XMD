...
async function start() {
    try {
        ...
        console.log(`🤖 VECTRA-MD using WA v${version.join('.')}, isLatest: ${isLatest}`);
        ...
        const Matrix = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return { conversation: "Vectra-XMD Cloud AI WhatsApp Bot" };
            }
        });
...
Matrix.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
        ...
        if (initialConnection) {
            console.log(chalk.green("✅ Connected Successfully Vectra-XMD Cloud AI 🤝"));
            ...
            try {
                ...
                if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_SEEN) {
                    ...
                    if (config.AUTO_STATUS_REPLY) {
                        const customMessage = config.STATUS_READ_MSG || '✅ Auto Status Seen Bot By VECTRA-MD';
...
async function init() {
    if (fs.existsSync(credsPath)) {
        ...
        if (config.SESSION_ID && config.SESSION_ID.startsWith("Vectra~")) {
            console.log("📥 Attempting to load Vectra session (GZIP compressed)...");
            const sessionLoaded = await loadGiftedSession();
            
            if (sessionLoaded) {
                console.log("✅ Vectra session loaded successfully!");
                await start();
            } else {
                console.log("❌ Failed to load Vectra session, falling back to QR code.");
                useQR = true;
                await start();
            }
